...
export default function OmLoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${OM_API}${endpoint}`, {
                method: 'POST',
                headers: {
    ...
    // Employee/Admin ke according signup page open karega
    const handleSignup = () => {
        if (loginType === 'employee') {
            router.push('/om/employee/signup');
        } else {
            router.push('/om/admin/signup');
        }
    };
                ...
                <div className="flex rounded-full bg-[#1c1c2e] p-1 mb-6">
                    {['employee', 'admin'].map((type) => (
                        <button
                            ...
                            className={`flex-1 py-1.5 text-sm rounded-full capitalize transition ${loginType === type
